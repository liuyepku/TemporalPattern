    ...
    )
{
    ...
    if (is_followed_by(is,'<',false))
    {
	// z.B. data/time</t             (in selber Datei)
	// z.B. data/time</data/t        (in selber Datei)
	// z.B. data/time<x.ddl/model/t  (in anderer Datei)

	// Analysiere partielle datei/collection/field Sequenz
	{
	    string s1,s2,s3;
	    is_followed_by(is,'/',false);
	    read_word_break(is," />:@",s1);
	    if (is_followed_by(is,'/',false))
	    {
		read_word_break(is," />:@",s2);
		if (is_followed_by(is,'/',false))
		    read_word_break(is," />:@",s3);
	    }
	    TRACE(s1<<'/'<<s2<<'/'<<s3);
	    if (s1.find(".")!=string::npos) // file/coll/field
	    {
		p_context->infile() = s1;
		if (!s2.empty()) p_context->coll() = s2;
		if (!s3.empty()) p_context->intag() = s3;
	    } 
	    else 
	    { 
		if (s2.empty()) // nur ein Arg s1 => field
		    p_context->intag() = s1;
		else // zwei Args coll/field
		{
		    if (s1!=table) 
			ERROR("Cannot change collection within same file.");
		    p_context->coll() = s1;
		    p_context->intag() = s2;
		}
	    }
